...
assign : ARRAY_STRING_VALUES Equal ((ARRAY_STRING_VALUES | Text |ARRAY_NUMERIC_VALUES) (Plus|Slash|Minus|Star)?)* SEIMIE_QO*;

decl : VarType ARRAY_STRING_VALUES (Equal value)? SEIMIE_QO*;

log : Console ((Text|ARRAY_STRING_VALUES) Plus?)+ RParen SEIMIE_QO*;

value: Text|(ARRAY_NUMERIC_VALUES)|One_Qoute_Term|Tow_Qoute_Term|Bool;
...
tag : ( div | button | h1 | img | p | javaScriptValue )*;
...
p: Lt P (props)* Gt ( tag | ( variable | ARRAY_STRING_VALUES |ARRAY_NUMERIC_VALUES)*) Lt Slash P Gt;
img: Lt Img src (props)* Slash Gt;
button : Lt Button (props)* Gt (ARRAY_STRING_VALUES |ARRAY_NUMERIC_VALUES)* Lt Slash Button Gt;

variable : LBrack ARRAY_STRING_VALUES RBrack;
